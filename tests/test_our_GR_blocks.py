...
def test_Blk_strength_at_freq_1():
    ...
    fft_size = 1024
    tb = gr.top_block()
    sig_s = analog.sig_source_c(samp_rate, analog.GR_COS_WAVE, 1000, 1)
...
def test_Blk_strength_at_freq_2():
    ...
    fft_size = 1024
    tb = gr.top_block()
    sig_s = analog.sig_source_c(samp_rate, analog.GR_COS_WAVE, 3000, 1)
    stream_to_vec = blocks.stream_to_vector(gr.sizeof_gr_complex, fft_size)
    saf = Blk_strength_at_freq(samp_rate, 1000, fft_size)
...
def test_Blk_strength_at_freq_3():
    ...
    fft_size = 1024
    tb = gr.top_block()
    sig_s = analog.sig_source_c(samp_rate, analog.GR_COS_WAVE, 4800, 1)
    stream_to_vec = blocks.stream_to_vector(gr.sizeof_gr_complex, fft_size)
