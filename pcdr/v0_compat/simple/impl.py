...
class OsmosdrReceiver:
    ...
    @typechecked
    def __init__(
        self, device_name: str, freq: float, *, device_id: Union[int, str] = 0
    ):
        """
        ...
        self.stream_to_vec = blocks.stream_to_vector(
            gr.sizeof_gr_complex, self.fft_size
        )
        self.streng = Blk_strength_at_freq(
            self.samp_rate, self.freq_offset, self.fft_size, 10
        )
