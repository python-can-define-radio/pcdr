...
from pcdr._internal.our_GR_blocks import (
    Blk_strength_by_mult,
    Blk_VecSingleItemStack,
    ProbeReadable,
...
from pcdr._internal.misc import (
    ...
    get_OsmocomArgs_TX,
    configureOsmocom,
    create_top_block_and_configure_exit,
    Startable,
    StopAndWaitable,
    Waitable,
    IFGainSettable,
...
class OsmoSingleFreqReceiver(Startable, StopAndWaitable):
    ...
    def __init__(self, device_args: str, freq: float):
        ...
        self.__streng = Blk_strength_by_mult(
            self._osmoargs.samp_rate, self.__freq_offset, chunk_size
        )
...
class OsmoSingleFreqTransmitter(
    Startable, StopAndWaitable, IFGainSettable, CenterFrequencySettable
):
    ...
    def __init__(self, device_args: str, freq: float):
        ...
        self.__constant_source = analog.sig_source_c(
            self._osmoargs.samp_rate, analog.GR_CONST_WAVE, 0, 0, 1
        )
...
@typechecked
def _pick_audio_source(
    audio_device: Optional[str],
    wavfile: Optional[Union[str, Path]],
    audio_sample_rate: float,
    repeat: bool,
) -> Union[audio.source, blocks.wavfile_source]:
    """
...
class AudioPlayer(Startable, StopAndWaitable, Waitable, ProbeReadable):
    ...
    @typechecked
    def connect_probe(
        self, *, location: Literal["wavfile_source", "audio_sink"], vecsize: int
    ):
...
class OsmoWBFMTransmitter(
    ...
    def __init__(
        ...
        *,
        ...
        """
        self._tb = create_top_block_and_configure_exit()
        self.__source = _pick_audio_source(
            audio_device, wavfile, audio_sample_rate, repeat
        )
        self._osmoargs = get_OsmocomArgs_TX(freq, device_args)
        self.__rational_resampler = filter.rational_resampler_fff(
            int(self._osmoargs.samp_rate), int(audio_sample_rate)
        )
        self.__wfm_tx = analog.wfm_tx(
            self._osmoargs.samp_rate, self._osmoargs.samp_rate
        )
        self._osmo = configureOsmocom(osmosdr.sink, self._osmoargs)
        self._tb.connect(
            self.__source, self.__rational_resampler, self.__wfm_tx, self._osmo
        )
...
class OsmoUnprocessedReceiver(Startable, StopAndWaitable, CenterFrequencySettable):
    ...
    def get_reading(
        self, block: bool = True, timeout: float = 2.0
